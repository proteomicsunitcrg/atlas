...
launch_nf_run () {
      ...
      if [[ $EXECUTOR == "slurm" ]]; then
         ...
         sbatch --output='/users/pr/proteomics/mygit/atlas-logs/atlas-slurm.out' --error='/users/pr/proteomics/mygit/atlas-logs/atlas-slurm.err' '/users/pr/proteomics/mygit/atlas-test/bin/submit_slurm.sh' $2 $WITH_TOWER -bg -with-report -work-dir $ATLAS_RUNS_FOLDER/$CURRENT_UUID --var_modif $3 --sites_modif $4 --fragment_mass_tolerance "$5" --fragment_error_units "$6" --precursor_mass_tolerance "$7" --precursor_error_units "$8" --missed_cleavages "$9" --output_folder "${10}" --instrument_folder "$INSTRUMENT_FOLDER" --search_engine "${12}" -profile ${EXECUTOR}_${13},$LAB --sampleqc_api_key ${14} --rawfile ${15} --test_mode $TEST_MODE --test_folder $ORIGIN_FOLDER --notif_email $NOTIF_EMAIL --enable_notif_email $ENABLE_NOTIF_EMAIL --lab $LAB 2>&1
