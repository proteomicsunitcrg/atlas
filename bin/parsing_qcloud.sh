...
get_openms_peptide_name() {
    ...
    # SIMPLE DIRECT DETECTION - bypass the complex logic
    ...
    if [[ "$sample_id" == *"QCD1"* ]]; then
        ...
        detected_qc_type="QCD2"
        echo "[DEBUG] Detected QC type: QCD2" >&2
    elif [[ "$sample_id" == *"QC01"* ]]; then
        detected_qc_type="QC01"
        echo "[DEBUG] Detected QC type: QC01" >&2
    elif [[ "$sample_id" == *"QC02"* ]]; then
        detected_qc_type="QC02"
        echo "[DEBUG] Detected QC type: QC02" >&2
    else
        detected_qc_type="QC01"  # Default fallback
        echo "[DEBUG] Defaulting to QC01" >&2
    fi
    
    echo "[CRITICAL DEBUG] Final detected QC type: $detected_qc_type" >&2
    
    # Determine mapping suffix
    local mapping_suffix=""
    if [[ "$detected_qc_type" == "QC01" ]]; then
        mapping_suffix=""
    elif [[ "$detected_qc_type" == "QC02" ]]; then
        mapping_suffix="_qc02"
    else
        mapping_suffix="_${detected_qc_type}"
    fi
    ...
    # Extract peptide mapping from config
    local openms_name=""
    if [[ -f "$config_file" ]]; then
        openms_name=$(awk -v section="$mapping_section" -v peptide="$simple_name" '
            $0 ~ section " = \\[" { in_section=1; next }
            in_section && (/^\s*\]/ || /^[[:space:]]*[a-zA-Z_][a-zA-Z0-9_]*[[:space:]]*=/) { in_section=0 }
            in_section && $0 ~ "\"" peptide "\":" { 
                gsub(/.*: *"/, "")
                gsub(/".*/, "")
                print $0
                exit
    ...
    if [[ -z "$openms_name" ]]; then
        ...
        echo "[DEBUG] No mapping found, using simple name: $openms_name" >&2
    fi
